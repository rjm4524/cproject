...
int main3()
{
	...
	scanf("%d", &staffnum);
		if (staffnum >= 10 && staffnum <= 99) {

		}
		else {
			printf("사원번호를 다시입력해주세요.");
			return 0;
		}
	printf("나이를 입력하세요.");
	scanf("%d", &age);
	...
	printf("부서코드를 입력하세요.");
	scanf(" %c", &department);
	if ((department >= 'a' && department <= 'z') || (department >='A' && department <= 'Z')) {
			...
			if (age < 18) {
				...
				if (Workhour > (3600 * 7)||money > (12000 * 7)) {
					...
					if (oversec >= 1) {
						...
						overmoney = (overtime / 60) * 100 + (overmin * 100); //초과급여 = 초과근무 시 + 초과근무 분 따로 계산후 합치기
						printf("사원번호:%d번\n나이:%d세\n부서명:인사팀%\n근무시간:%d시%d분\n초과근무 시간:%d시 %d분\n부양가족수:%d명\n신용카드 사용액:%d원\n체크카드 사용액:%d원\n기본급여 :%d\n초과급여:%d\n",
							staffnum, age, hour, min, overhour, overmin, family, card, checkcard,money,overmoney);
					}
